 ...
 */
class NoodleView extends Object {
 ...
 */
	protected $_passedVars = array(
		'viewVars', 'autoLayout', 'ext', 'helpers', 'view', 'layout', 'name', 'noodleThemePath', 'theme',
			...
			for ($i = 0; $i < $count; $i++) {
				if (strpos($paths[$i], DS . 'Plugin' . DS) === false
					&& strpos($paths[$i], DS . 'Cake' . DS . 'View') === false) {
						...
						$themePaths[] = $paths[$i] . 'Themed'. DS . $this->noodleThemePath . DS . $this->theme . DS;
					}
			}
